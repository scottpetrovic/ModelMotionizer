...
import { BufferGeometry, Float32BufferAttribute, Mesh, Box3, MeshBasicMaterial, SkeletonHelper } from 'three'
...
export class StepLoadSkeleton extends EventTarget
{
    ...
    addEventListeners()
    {
        this.ui.dom_load_skeleton_button.addEventListener('click', () => {
            ...
            this.loader.load( this._skeleton_type, (gltf) => {
              ...
              let skeleton_geometry = this._buffer_geometry_from_armature(this.loaded_armature);
              let temp_skeleton_mesh = new Mesh(skeleton_geometry, new MeshBasicMaterial({color: 0x00ff00}));
              let bounding_box = new Box3().setFromObject(temp_skeleton_mesh);
    ...
    _buffer_geometry_from_armature(armature)
    {
        ...
        armature.traverse((bone) => {
            if(bone.type === 'Bone')
            {
                let bone_world_pos = Utility.world_position_from_object(bone);
