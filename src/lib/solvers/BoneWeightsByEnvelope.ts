...
export class BoneWeightsByEnvelope 
{
    ...
    private bone_names_with_errors: Array<string> = [];
    private bone_vertices_with_errors: Array<Vector3> = [];
    ...
    public test_bones_outside_in_mesh(): [string[], Vector3[]]
    {
        this.bones_master_data.forEach( (bone: BoneCalculationData, bone_index: number) => {
            if (bone.supports_envelope_calculation)
            {
                ...
                const [intersection_distances, intersection_vertexes] = this.intersections_for_geometry(rays_to_cast, intersection_plane_mesh);
    ...
    public calculate_indexes_and_weights()
    {
        ...
        const closest_vertex_list: Array<number> = [] // vertex index linked list
        const closest_bone_list: Array<number> = []; // bone index linked list

        vertex_indexes_without_bones.forEach( (vertex_index, idx) => {
            let closest_bone_index: number = find_closest_bone_index_from_vertex_index(vertex_index, this.geometry, this.bones_master_data) 
        ...
        return [ final_skin_indices, final_skin_weights ]
    }

    private create_envelope_mesh_for_bone(bone)
    {
        let envelope_color = 0xaaaaff

        let envelope_geometry: BoneEnvelopeGeometry = new BoneEnvelopeGeometry()
        ...
        let bone_start: Bone = bone.bone_object
        let bone_end: Bone = bone_start.children[0] as Bone
    ...
    private expand_envelope_mesh_to_fit_bone(envelope_mesh: Mesh, intersections, bone: BoneCalculationData)
    {
        let bone_start = bone.bone_object
        let bone_end = bone_start.children[0]
    ...
    private intersections_for_geometry(rays_to_cast, debug_bone_plane_mesh) {
        const intersection_distances: Array<number> = []; // ray cast around the object and find how far out the mesh goes
        const intersection_vertexes: Array<Vector3> = []; // output for potentially showing in debugger
        ...
        return [intersection_distances, intersection_vertexes];
    }
     ...
     * @returns 
     */
    private calculate_final_weights(bones_master_data, geometry_vertex_count) 
    { 
        const final_skin_indices: Array<number> = [];
        const final_skin_weights: Array<number> = [];
        ...
        for (let i = 0; i < geometry_vertex_count; i++) {
            ...
            const bones_for_vertex: Array<number> = [];
            const weights_for_vertex: Array<number> = [];
...
function vertices_not_assigned_to_bone(geometry_vertex_count: number, bones_master_data: Array<BoneCalculationData>) 
{
    const vertex_indexes_without_bones: Array<number> = [];
    ...
    for (let i = 0; i < geometry_vertex_count; i++) 
    {
        ...
        if (does_vertex_exist_in_bones === false) 
        {
...
function find_closest_bone_index_from_vertex_index(vertex_index, geometry, bones: Array<BoneCalculationData> ) 
{
